 ...
 *********************************/
int is_all_caps(char *text) {
  int ii;
  int all_caps = 1;
  for (ii = 0; ii < strlen(text); ii++) {
    if (isalpha(text[ii]) != 0 && islower(text[ii]) != 0) all_caps = 0;
  }
  return all_caps;
}
...
int test_uz = 1;
int CENTER_LINE = 5000;
int detect_toc_section(int bk) { // don't mark items that come after TOC sections
  int ret = 0;
  char *text = block_array[bk].text;
  if (test_uz
      && (block_array[bk].last_word - block_array[bk].first_word) <= 3
      && (block_array[bk].my_x2 - block_array[bk].my_x1) < 2000
      && abs(block_array[bk].my_x2 + block_array[bk].my_x1 - 2 * CENTER_LINE) < 800
      && is_all_caps(block_array[bk].text)
      )
    {
      fprintf(stderr,"BLOCKINFO: :%s: %d:%d:\n", block_array[bk].text, block_array[bk].my_x2, block_array[bk].my_x1);
      ret = 1;
    }
  if (!text) {
