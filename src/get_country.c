...
char *get_default_data_country_from_sysconfig(MYSQL *conn) {
  static char query[200000];
  char *default_data_country = "USA";

  sprintf(query,"select value from dealthing_sysconfig \n\
                        where param_name='default_data_country' "
  );
  ...
  if (nn > 0) {
      ...
      default_data_country = strdup(sql_row[0]);
  }
...
int get_country(MYSQL *conn, int did) {

  MYSQL_RES *sql_res;
  MYSQL_ROW sql_row;
  static char query[10000];

  sprintf(query,"select ct.data_country, cp.data_country, co.data_country, d.id, t.id, p.id, o.id \n\
                          ...
                          left join deals_org_folder_country as co on (co.org_id = o.id) \n\
                          left join deals_org_folder_country as cp on (cp.folder_id = p.id) \n\
                          left join deals_org_folder_country as ct on (ct.folder_id = t.id) \n\
                          where d.id = %d "
	    , did
	  );
  ...
  char *data_country = get_default_data_country_from_sysconfig(conn);
  sql_row = mysql_fetch_row(sql_res);
  if (sql_row) {
    if (sql_row[0]) { // try t country first
      data_country = strdup(sql_row[0]);
    } else if (sql_row[1]) { // try p country next
      data_country = strdup(sql_row[1]);
    } else if (sql_row[2]) { // try o country next
      data_country = strdup(sql_row[2]);
    }
  } else {
    fprintf(stderr,"Error: no sql_row for doc=%d:\n",doc_id);
  }

  if (debug) fprintf(stderr,"COUNTRY :%s:\n",data_country);
  printf("%s",data_country);
  return 0;
} // get_country()
