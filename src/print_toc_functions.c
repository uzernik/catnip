...
int create_and_compare_toc_pairs() { // compare each pair using Levenshtein
  int ii, jj;
  int kk = 0;
  int wrap_LD_count = 0;
  for (ii = 0; ii < item_no; ii++) {
    if (strlen(item_array[ii].norm_header) < legit_header_length && strlen(item_array[ii].norm_header) > 2) {    // only long enough
	...
	if (strlen(item_array[jj].norm_header) < legit_header_length  && strlen(item_array[jj].norm_header) > 2) {    // only long enough
	  wrap_LD_count++;
	...
	pair_no = kk++;
      ...
      if (same_page > 5) { // once found a header with 6 brothers on the same page then we are done, SAVES TONS OF TIME (30 seconds)!!!!
	break;
      }
    } // only long enough
  } // for ii
...
// processed by lex
// output to rest file
int input_split_and_clean_rest_file(int ii1, int ii2, char *rest_name, int first_toc) {
...
} // input_split_and_clean_rest_file(int ii1, int ii2)
...
int transfer_toc_toc_into_buff(int first_toc,int first_rest, char *toc_toc_name, char toc_toc_buff[]) {
  int ii;
  static char buff[3000];
  fprintf(stderr,"PRINTING TOC_TOC:%d:%d:\n",first_toc,first_rest);
  for (ii = first_toc; ii < first_rest; ii++) {
    sprintf(buff,"$%s %s %s\n", item_array[ii].title,item_array[ii].section,item_array[ii].case_norm_header);
  ...
  return 0;
} // transfer_toc_toc_into_buff(int first_toc,int first_rest) 
...
int import_results_file(char *results_name, struct NYU_Struct nyu_results_array[], char *results_ptr) {
  ...
  int ii;
  for (ii = 0; ii < nyu_results_no; ii++) {
    while (nyu_results_array[ii].nyu_buff[0] == ' ') { // if the first char is " " (used to be "\n"), then move up by one
...
int align_by_toc_toc() {
  ...
  create_and_compare_toc_pairs(); // step 2:  find matching toc header pairs, stop after a sequence of 5 -- what is not found???
  current_timestamp(902);
  fprintf(stderr,"ORG3: find first pair\n");
  int first_pair = find_first_good_pair(); // step 3: get the first in the sequence
  ...
  transfer_toc_toc_into_buff(ii,jj, toc_toc_name, toc_toc_buff); // step 4: get the toc list into a physical file
  int bii = item_array[ii].my_loc;
  int bjj = item_array[jj].my_loc;
  int position_no = input_split_and_clean_rest_file(bii, bjj, rest_name, ii); // step 5: using a sequence of LEX, process the aaa5 file into a clean file (rest_name)
